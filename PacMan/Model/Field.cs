...
namespace PacMan.Model
{
	public sealed class Field : IField
    {
		...
		internal ICell this[int x, int y]
	    {
		    get
		    {
	    ...
	    public bool AbleToMove(int x, int y)
	    {
		    return Layers.Values.All(layer => layer.AbleToMove(x, y));
	    }

	    public void NotifyMovement(int x, int y, Direction direction, IEntity caller)
		{
	    ...
	    public void ResetActiveCell(int fromX, int fromY, int toX, int toY)
		{
	    ...
	    public void RemovePassiveCell(int x, int y)
		{
			Remove(Layers[CellType.Passive][x, y]);
		}

	    private void Remove(ICell cell)
		{
			if (cell == null)
			{
	    ...
	    public void RelocateCell(ICell cell, int toX, int toY)
	    {
            if (cell == null)
            {
