...
namespace PacMan.Model.Cells
{
	...
	{
		private static readonly Dictionary<int, Func<IField, int, int, Cell>> Creators = new Dictionary<int, Func<IField, int, int, Cell>>();
		...
		private static void Register(int cellRepresentationChar, Func<IField, int, int, Cell> cellCreator)
		{
			Creators.Add(cellRepresentationChar, cellCreator);
		}

		public Cell Parse(int cellRepresentationChar, IField field, int x, int y)
		{
			return Creators[cellRepresentationChar] == null ? null : Creators[cellRepresentationChar](field, x, y);
		}

        public Cell GetSpaceCell(IField field, int x, int y)
		{
